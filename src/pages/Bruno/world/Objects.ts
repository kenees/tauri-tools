...
export default class Objects {
  ...
  items: any[];
  floorShadows: THREE.Object3D[];
  parsers: any;
  merge: any;
  constructor(_options: { materials: any; time: any; resources: any; physics: any; shadows: any; sounds: any; debug: any }) {
    // Options
  ...
  setParsers() {
    ...
    this.parsers.default.apply = (_mesh: any) => {
      // Create clone mesh with normal material
      const mesh = _mesh.clone();
      mesh.material = this.materials.shades.items.items.white;
  ...
  setMerge() {
    ...
    this.merge.add = (_name: string, _mesh: any) => {
  ...
  // 获取转换后的mesh
  getConvertedMesh(_children: any[], _options: any = {}) {
    const container = new THREE.Object3D();
    const center = new THREE.Vector3();

    // 检查每个 base child
    const baseChildren = [..._children];
    console.log("baseChildren: ", baseChildren);
    ...
    for (const _child of baseChildren) {
      ...
      if (_child instanceof THREE.Mesh) {
        console.log("Mesh: ", _child.name);
        // Find parser and use default if not found
        let parser = this.parsers.items.find((_item: any) => _child.name.match(_item.regex));
        ...
        console.log("Mesh after parser: ", mesh);
        // Add to container
        container.add(mesh);
      }
    }
  ...
  add(_options: any) {
    ...
    if (_options.soundName) {
      object.collision.body.addEventListener("collide", (_event: any) => {
