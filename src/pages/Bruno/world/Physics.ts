...
export default class Physics {
  ...
  constructor(_options: { config: any; debug: any; scene?: any; time: any; sizes: any; controls: any; sounds: any }) {
  ...
  setCar() {
    ...
    this.car.create = () => {
       ...
       * Sound
       */
      this.car.chassis.body.addEventListener("collide", (_event: any) => {
     ...
     * Actions
     */
    this.controls.on("action", (_name: string) => {
      switch (_name) {
        case "reset":
          this.car.recreate();
          break;
    ...
    if (this.debug) {
      ...
      this.car.debugFolder
        .add(this.car.options, "controlsSteeringMax")
        .step(0.001)
        .min(0)
        .max(Math.PI * 0.5)
  ...
  addObjectFromThree(_options: any) {
    // Set up
    const collision: any = {};
